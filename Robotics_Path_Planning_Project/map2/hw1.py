...
##start_goal_pair_number = 4
nodes_expanded = []
length = len(map[2])
##length = 5
timearr = []

for start_goal_pair_number in range(0, length):
	...
	for j in range(0, ylen + 1):
	    for i in range(0, xlen + 1):
	        if j == 0:
	            ...
	            # Bottom left cell openList
	            if M[j - 1, i - 1] == 0:
	                N[j][i].neighbors.add(N[j - 1][i])
	                N[j][i].neighbors.add(N[j][i - 1])
	                N[j][i].neighbors.add(N[j - 1][i - 1])
	            # Bottom right cell openList
	            if M[j - 1, i] == 0:
	                N[j][i].neighbors.add(N[j - 1][i])
	                N[j][i].neighbors.add(N[j][i + 1])
	                N[j][i].neighbors.add(N[j - 1][i + 1])
	            # Top left cell openList
	            if M[j, i - 1] == 0:
	                N[j][i].neighbors.add(N[j + 1][i])
	                N[j][i].neighbors.add(N[j][i - 1])
	                N[j][i].neighbors.add(N[j + 1][i - 1])
	            # Top right cell openList
	            if M[j, i] == 0:
	                N[j][i].neighbors.add(N[j + 1][i])
	                N[j][i].neighbors.add(N[j][i + 1])
	                N[j][i].neighbors.add(N[j + 1][i + 1])
	...
	print("A* nodes: ", AstarExpandednodes)
	...
	resultFile = directory + "/coordinates" + str(start_goal_pair_number) + ".txt"

	with open(resultFile, 'a') as the_file:
		the_file.truncate(0) 
		for i in range(0, len(path)):
			the_file.write(str(path[i][0])+" "+ str(path[i][1])+ "\n")  
	...
	# Clear parents and g values from Vertice Matrix
	for j in range(0, ylen+1):
	    for i in range(0, xlen+1):
	        N[j][i].p = None
	        N[j][i].g = 0
	...
	# TODO: fix function if problem with logic
	def grid(x, y):
	    x_index = int(x)
	    y_index = int(y)
	    ...
	    # Vertex is blocked if any of 4 surrounding blocks are blocked so return False in this case
	    # if M[y_index][x_index] or M[y_index-1][x_index] or M[y_index][x_index-1] or M[y_index-1][x_index-1]:
	    #      return True
	    return False
	...
	def LineOfSight(s, s_prime_inner):
	    ...
	    if dx < 0:
	        ...
	        sx = 1
	    if dx >= dy:
	        # TODO: add int() or something so indices match
	        while x0 != x1:
	            f = f + dy
	            if f >= dx:
	...
	timearr.append((end_time, end_time2))
	...
	print("FDA nodes: ", FDAExpandednodes)

	nodes_expanded.append((AstarExpandednodes, FDAExpandednodes))
	print(timearr[start_goal_pair_number])
	...
	while n != startVertex:
	    n = n.p
	    pathFDA.append((n.x,n.y))

	temp = []
	pathLen = len(pathFDA)
	for k in range(0, pathLen):
		temp.append(pathFDA[pathLen-1-k])
	pathFDA =temp
